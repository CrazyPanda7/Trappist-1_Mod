...
public abstract class ChunkGeneratorBase implements IChunkGenerator
{
    ...
    public ChunkGeneratorBase(World world, long seed)
    {
        this.world = world;
    ...
    {
        ChunkGeneratorBase.generatePocket(world, rand, xx, zz, pocket, ignoreBiomes, 17 + rand.nextInt(10) + rand.nextInt(5), 3 + rand.nextInt(5));
    }
    ...
    protected static void generatePocket(World world, Random rand, int xx, int zz, IBlockState pocket, Set<Biome> ignoreBiomes, int y, int radius)
    {
        ...
        if (ChunkGeneratorBase.canGeneratePocket(world, rand, xx, zz, pos, ignoreBiomes, y))
        {
            int x = pos.x;
            int cy = pos.y;
            int z = pos.z;

            if (ChunkGeneratorBase.checkPocketPresent(world, x, cy, z, radius, pocket))
            {
                return;
            }
            ...
            for (int bx = -radius; bx <= radius; bx++)
            {
                for (int by = -radius + 2; by <= radius - 2; by++)
                {
                    ...
                    for (int bz = -radius; bz <= radius; bz++)
                    {
                        ...
                        {
                            if (ChunkGeneratorBase.checkBlock(world, new BlockPos(bx + x - 1, by + cy, bz + z), pocket))
                            {
                                continue;
                            }
                            if (ChunkGeneratorBase.checkBlock(world, new BlockPos(bx + x + 1, by + cy, bz + z), pocket))
                            ...
                            if (ChunkGeneratorBase.checkBlock(world, new BlockPos(bx + x, by + cy - 1, bz + z), pocket))
                            {
                                continue;
                            }
                            if (ChunkGeneratorBase.checkBlock(world, new BlockPos(bx + x, by + cy, bz + z - 1), pocket))
                            {
                                continue;
                            }
                            if (ChunkGeneratorBase.checkBlock(world, new BlockPos(bx + x, by + cy, bz + z + 1), pocket))
                            ...
                            if (ChunkGeneratorBase.checkBlockAbove(world, new BlockPos(bx + x, by + cy + 1, bz + z)))
                            {
                                continue;
                            }
