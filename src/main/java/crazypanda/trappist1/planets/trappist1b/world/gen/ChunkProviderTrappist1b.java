...
import crazypanda.trappist1.core.Trappist1Biomes;
...
public class ChunkProviderTrappist1b extends ChunkProviderBase{

	public static final IBlockState BLOCK_TOP = ModBlocks.B_SURFACEROCK.getDefaultState();
	public static final IBlockState BLOCK_FILL = ModBlocks.B_SUBROCK.getDefaultState();
	public static final IBlockState BLOCK_LOWER = ModBlocks.B_ROCK.getDefaultState();
	...
	private Biome[] biomesForGeneration = { Trappist1Biomes.TRAPPIST1B };
	...
	public ChunkProviderTrappist1b(World par1World, long par2, boolean par4) {
	...
	public void setBlocksInChunk(int chunkX, int chunkZ, ChunkPrimer primer) {
		...
		for (int x = 0; x < ChunkProviderTrappist1b.CHUNK_SIZE_X; x++) {
			for (int z = 0; z < ChunkProviderTrappist1b.CHUNK_SIZE_Z; z++) {
				...
				for (int y = 0; y < ChunkProviderTrappist1b.CHUNK_SIZE_Y; y++) {
					if (y < ChunkProviderTrappist1b.MID_HEIGHT + yDev) {
	...
	public Chunk generateChunk(int x, int z) {
		...
		final byte b = (byte) Biome.getIdForBiome(Trappist1Biomes.TRAPPIST1B);
				...
				for (int x = 0; x < ChunkProviderTrappist1b.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderTrappist1b.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x
								* ChunkProviderTrappist1b.CHUNK_SIZE_X + x, cz * ChunkProviderTrappist1b.CHUNK_SIZE_Z + z)
								/ ChunkProviderTrappist1b.CRATER_PROB) {
